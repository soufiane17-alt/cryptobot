...
def get_signal(symbol="BTC/USDT"):
    try:
        ...
        # === SCORING FINAL STRICT (très exigeant) ===
        score = 45

        # RSI très strict
        if last_rsi < 20: score += 35
        elif last_rsi < 27: score += 22
        elif last_rsi > 80: score -= 35
        elif last_rsi > 72: score -= 25

        # MACD (doit être très clair)
        if last_macd > last_macd_signal and macd.iloc[-2] <= macd_signal.iloc[-2]:
            score += 30
        if last_histogram > 0 and histogram.iloc[-2] <= 0:
            score += 15

        # Bollinger (forte cassure seulement)
        if last_price < last_lower_bb: score += 26
        elif last_price > last_upper_bb: score -= 24

        # EMA50 (tendance claire)
        if last_price > last_ema50: score += 15
        else: score -= 12

        # Bonus uniquement si convergence parfaite
        if last_rsi < 25 and last_price < last_lower_bb and last_macd > last_macd_signal:
            score += 22

        score = min(100, max(0, int(score)))

        # === DÉCISION TRÈS STRICTE ===
        if score >= 90 and last_rsi < 28:
            signal = "BUY"
            reason = f"Signal TRÈS FORT {score}/100"
        elif score >= 88 and last_rsi > 75:
            signal = "SELL"
            reason = f"Signal TRÈS FORT {score}/100"
        elif score >= 82:
