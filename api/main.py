...
<div style="max-width:1350px; margin: 0 auto; padding: 20px;">
  ...
  <div class="card">
    ...
    <div id="positions" style="min-height:60px;"></div>
  </div>
...
<script>
  ...
  async function update() {
    ...
    // Positions ouvertes
    let posHtml = '<p style="color:#888;">Aucune position ouverte</p>';
    if (Object.keys(data.portfolio.positions).length > 0) {
      posHtml = '<ul>';
      for (let [sym, p] of Object.entries(data.portfolio.positions)) {
        if (p.amount > 0) posHtml += `<li><strong>${sym}</strong> : ${p.amount} unités @ $${p.entry_price}</li>`;
      }
      posHtml += '</ul>';
    }
  ...
  function showAdmin() {
    ...
    if (code === "admin") {
      alert("✅ Accès Admin autorisé (page en cours de développement)");
    } else {
